...
public class BatchIntegrationTest {
    ...
    public void executeChunk_Success() throws Exception {
        ...
        // BasicProgressManagerで出力する件数（remaining count）やChunkProgressLogListenerで出力する件数（write count）
        // はStepContext#getMetrics()のREAD_COUNTから算出しているが、Metricsの更新タイミングは実装依存になっている。
        // 現在テストで使用するPayaraではjBeretと異なりtemWriteListenerの呼び出し後に更新されるため、READ_COUNTが
        // 実際の処理済み件数とは一致しない状態になっているが、ここでは実装に合わせて検証するようにしている。
        // （以降のテストケースでも同様）
        ...
        assertThat(messages, contains(
               ...
               startsWith("INFO progress chunk progress. write count=[0]"),
               allOf(
                       ...
                       containsString("remaining count: [25]")
               ),
    ...
    public void executeMultiStep() throws Exception {
        ...
        assertThat(messages, contains(
               ...
               allOf(
                       ...
                       containsString("current tps:"),
                       containsString("estimated end time:"),
                       containsString("remaining count: [10]")
               ),
    ...
    public void testOperationLogBatchlet() throws Exception {
        ...
        assertThat("運用者向けのメッセージと合わせてスタックトレースも出力されていること",
               InMemoryAppender.getLogMessages("ALL"), Matchers.hasItem(allOf(
                       startsWith("ERROR operator ファイルの読み込みに失敗しました。ファイルが存在するか確認してください。"),
    ...
    public void testSystemError() throws Exception {
        ...
        assertThat("スタックトレースが出力されていること",
               ...
               Matchers.hasItem(containsString("StackOverflowError")));
